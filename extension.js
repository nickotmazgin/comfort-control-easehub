...
function _spawn(cmdline) {
  try { GLib.spawn_command_line_async(cmdline); }
  catch (e) { log('[EaseHub] spawn error: ' + e); Main.notify('EaseHub', 'Failed to run: ' + cmdline); }
}
...
function _which(name) {
  try { return GLib.find_program_in_path(name) !== null; } catch (_) { return false; }
}
...
class EaseHubIndicator extends PanelMenu.Button {
  ...
  _buildMenu() {
    ...
    // Updates — explicit consent + hold-open
    ...
    // About & Donate (simple, opens URLs)
