...
# Build a refined tri‑color collage from screenshots.
# - Crops/centers tiles to uniform size
# - Adapts columns/rows to image count (2 or 3 columns)
# - Adds subtle border/padding
# - Applies a soft tri‑color gradient overlay (blue→green→orange)
...
montage "$WORK"/tile_*.png \
  -background "#0f1116" \
  -bordercolor "#232737" -border $BORDER \
...
# Tri‑color gradient: blue→green on left half, green→orange on right half
G1="$WORK/grad_l.png"
G2="$WORK/grad_r.png"
GRAD="$WORK/grad.png"
...
convert "$GRAD" -alpha set -channel a -evaluate set 45% "$GRAD"

# Optional gentle vignette for focus
VIG="$WORK/vig.png"
convert -size ${W}x${H} radial-gradient:black-transparent -alpha set \
  -channel a -evaluate divide 2 +channel \
  -fill none -colorize 0 "$VIG"

# Compose: base + gradient (softlight) + subtle vignette (over)
OUT_TMP="$WORK/collage.png"
convert "$BASE" "$GRAD" -compose softlight -composite "$OUT_TMP"
convert "$OUT_TMP" "$VIG" -compose over -composite "$OUT_TMP"
